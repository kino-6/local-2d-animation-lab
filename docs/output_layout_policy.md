...
## Adopted MVP Exception

Small, reviewable game asset packages may be written to:

```text
outputs/adoptable/<route-name>/
```

Use this only for deliberately adopted artifacts with a stable manifest and review notes, not for
exploratory PDCA runs. Exploratory generation still belongs under `outputs/<timestamp>/...`.
